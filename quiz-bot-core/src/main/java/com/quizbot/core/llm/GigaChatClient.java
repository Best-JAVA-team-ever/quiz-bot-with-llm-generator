...
public class GigaChatClient implements LlmClient {
    ...
    public GigaChatClient(@Value("${gigachat.api.key}") String apiKey,
    ...
    @Override
    public CompletableFuture<Question> generateQuestion(List<String> topics,
            Integer difficulty) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                meterRegistry.counter("llm.api.errors", "operation",
                        "generate_question").increment();
    ...
    @Override
    public CompletableFuture<String> generateExplanation(String questionText,
        ...
        return CompletableFuture.supplyAsync(() -> callSimple(prompt,
                "generate_explanation"));
    }

    @Override
    public CompletableFuture<String> generateHint(String questionText) {
        String prompt = String.format("Дай небольшую подсказку для вопроса: '%s', неназывая правильный ответ.",
                questionText);
        return CompletableFuture.supplyAsync(() -> callSimple(prompt,
                "generate_hint"));
    }
    ...
    public CompletableFuture<List<DifficultyUpdate>> suggestDifficultyUpdates(List<QuestionWithStats> stats) {
        ...
        promptBuilder.append(
                "Проанализируй статистику ответов на вопросы викторины и предложи новые уровни сложности (от 1 до 5).\n");
        promptBuilder.append(
                "Формат ответа строго JSON массив объектов: [{\"questionId\": \"...\", \"newDifficulty\": ...}]\n\n");
        promptBuilder.append("Статистика:\n");
        for (QuestionWithStats stat : stats) {
            promptBuilder.append(
                    String.format("ID: %s, Текст: %s, Всего ответов: %d, Правильных: %d, Текущая сложность: %d\n",
                            stat.question().id(), stat.question().text(), stat.totalCount(),
                            stat.correctCount(), stat.question().difficulty()));
        }

        return CompletableFuture.supplyAsync(() -> {
            try {
                String response = callGigaChatWithMetrics(promptBuilder.toString(),
                        "suggest_difficulty");
                JsonNode root = objectMapper.readTree(response);
                String content = root.path("choices").get(0).path("message").path("content").asText();
    ...
    private String callSimple(String prompt, String operation) {
        try {
            ...
            log.warn("Simple LLM generation failed for operation {}: {}", operation,
                    e.getMessage());
    ...
    private String callGigaChatWithMetrics(String prompt, String operation) {
        ...
        try {
            ...
            sample.stop(meterRegistry.timer("llm.api.duration", "operation", operation,
                    "status", "success"));
            meterRegistry.counter("llm.api.calls", "operation", operation, "status",
                    "success").increment();
            return result;
        } catch (Exception e) {
            sample.stop(meterRegistry.timer("llm.api.duration", "operation", operation,
